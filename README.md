...
- TTA-VoLTE readiness 확인: 한국 UICC 이동성 VoLTE 단말 규격 관점의 정적 evidence와 미확인 항목 분리
...
`TTA-VoLTE Assessment` 섹션은 TTA의 `사업자 간 UICC 이동성 제공을 위한 VoLTE 단말 규격(TTAK.KO-06.0357)` 관점에서 ADB로 확인 가능한 readiness evidence를 분리합니다.

확인하는 항목:

- LTE RAT 허용 여부
- Android IMS service 설정 및 바인딩 여부
- carrier VoLTE/video/WFC config 조회 결과 또는 조회 불가 사유
- IMS package 및 MMTEL/video 관련 evidence
- OMA-DM/IMS MO provisioning component 후보
- `TTA-VoLTE/1.0` SIP User-Agent marker 관측 여부
- USIM 기반 UICC mobility/runtime 검증 가능 여부

주의:

- `TTA-VoLTE readiness partially evidenced`는 TTA-VoLTE 인증 또는 통신사 화이트리스트 등록을 의미하지 않습니다.
- `TTA-VoLTE/1.0` User-Agent, SIP/SDP, AMR-WB/H.263/H.264 codec 협상, IMS MO provisioning 완료 여부는 일반 stock ADB만으로는 보통 확인할 수 없습니다.
- 실제 대응 여부 확정에는 USIM 삽입 후 IMS registration, VoLTE 발착신, 영상통화, SMS/MMS, 사업자 간 UICC 이동성 테스트가 필요합니다.
